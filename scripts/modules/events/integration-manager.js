...
import { EVENT_TYPES, DEFAULT_CONFIG, validateEventPayload, createEventPayload } from './types.js';
...
export class IntegrationManager {
  ...
  async initialize(config = {}) {
    ...
    try {
      ...
      const initPromises = Array.from(this.integrations.values()).map(async (integration) => {
        try {
          await integration.initialize(this.config);
          log('info', `Integration ${integration.getName()} initialized`);
        } catch (error) {
          log('error', `Failed to initialize integration ${integration.getName()}:`, error.message);
    ...
    try {
      ...
      const shutdownPromises = Array.from(this.integrations.values()).map(async (integration) => {
        try {
          ...
          log('error', `Error shutting down integration ${integration.getName()}:`, error.message);
        }
      });
    ...
    try {
      ...
      for (const [eventType, handlers] of this.handlers.entries()) {
        const filteredHandlers = handlers.filter(handler => 
          !(handler.integration && handler.integration.getName() === integrationName)
        );
      ...
      log('error', `Error unregistering integration ${integrationName}:`, error.message);
      throw error;
    }
  }
  ...
  async emit(eventType, data, context) {
    if (!this.initialized) {
      log('warn', 'IntegrationManager not initialized, event will be ignored:', eventType);
      return;
    }

    if (this.isShuttingDown) {
      log('warn', 'IntegrationManager is shutting down, rejecting event:', eventType);
      return;
    }
  ...
  getStats() {
    return {
      ...
      registeredHandlers: Array.from(this.handlers.values())
    ...
    try {
      ...
      for (const middleware of this.middleware) {
        try {
          ...
          log('error', `Middleware error for event ${eventPayload.type}:`, error.message);
      ...
      this.stats.eventsFailed++;
      log('error', `Error processing event ${eventPayload.type}:`, error.message);
      throw error;
    }
  }
  ...
  async _executeHandlers(eventType, payload, handlers) {
    ...
    if (concurrentHandlers.length > 0) {
      const batches = [];
      for (let i = 0; i < concurrentHandlers.length; i += this.config.maxConcurrentHandlers) {
        batches.push(concurrentHandlers.slice(i, i + this.config.maxConcurrentHandlers));
      }
      ...
      for (const batch of batches) {
        ...
        results.forEach((result, index) => {
          if (result.status === 'rejected') {
            ...
            const integrationName = handler.integration ? handler.integration.getName() : 'unknown';
            log('error', `Handler failed for ${eventType} (${integrationName}):`, result.reason.message);
          } else {
    ...
    for (const handlerWrapper of sequentialHandlers) {
      try {
        ...
        this.stats.handlersFailed++;
        const integrationName = handlerWrapper.integration ? handlerWrapper.integration.getName() : 'unknown';
        log('error', `Sequential handler failed for ${eventType} (${integrationName}):`, error.message);
      }
    }
  }
  ...
  async _executeHandler(eventType, payload, handlerWrapper) {
    const timeout = handlerWrapper.options.timeout || this.config.handlerTimeout;
  ...
  async _processBatch() {
    ...
    const promises = events.map(eventPayload => this._processEvent(eventPayload));
