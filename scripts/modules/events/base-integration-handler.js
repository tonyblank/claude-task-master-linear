...
export class BaseIntegrationHandler {
  ...
  constructor(name, version, config = {}) {
    if (this.constructor === BaseIntegrationHandler) {
      throw new Error('BaseIntegrationHandler is abstract and cannot be instantiated directly');
    }
  ...
  async initialize(config = {}) {
    ...
    try {
      ...
      log('info', `Integration ${this.name} v${this.version} initialized successfully`);
    } catch (error) {
      log('error', `Failed to initialize integration ${this.name}:`, error.message);
  ...
  async shutdown() {
    ...
    try {
      ...
      if (this.activeOperations.size > 0) {
        log('warn', `Integration ${this.name} shutdown with ${this.activeOperations.size} active operations`);
      }
      ...
      log('error', `Error during integration ${this.name} shutdown:`, error.message);
    } finally {
      this.isShuttingDown = false;
    }
  }
  ...
  async handleEvent(eventType, payload) {
    if (!this.isEnabled()) {
      log('debug', `Integration ${this.name} is disabled, skipping event ${eventType}`);
      return null;
    }

    if (this.isShuttingDown) {
      log('warn', `Integration ${this.name} is shutting down, rejecting event ${eventType}`);
    ...
    try {
      ...
      log('debug', `Integration ${this.name} successfully handled event ${eventType}`);
      return result;
    } catch (error) {
      log('error', `Integration ${this.name} failed to handle event ${eventType}:`, error.message);
      throw error;
    } finally {
  ...
  async retry(operation, retryConfig = {}) {
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        log('warn', `Integration ${this.name} operation failed (attempt ${attempt}/${config.maxAttempts}), retrying in ${delay}ms:`, error.message);
  ...
  async _routeEvent(eventType, payload) {
    ...
    log('debug', `Integration ${this.name} has no handler for event type ${eventType}`);
    return null;
  }
