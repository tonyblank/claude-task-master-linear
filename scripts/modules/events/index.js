	...
	if (eventType.startsWith('task:') && payload.task) {
		payload.enriched = {
			hasSubtasks:
				...
				payload.task.subtasks.length > 0,
			hasDependencies:
				Array.isArray(payload.task.dependencies) &&
				payload.task.dependencies.length > 0,
...
export function createFilterMiddleware(filterConfig = {}) {
	return function filterMiddleware(eventType, payload) {
		...
		if (
			filterConfig.excludeEventTypes &&
			filterConfig.excludeEventTypes.includes(eventType)
		) {
		...
		if (eventType.startsWith('task:') && payload.task) {
			if (
				filterConfig.excludeInternalTasks &&
				payload.task.title?.startsWith('_')
			) {
				return null;
			}
		...
		// Filter by source
		if (
			filterConfig.allowedSources &&
			!filterConfig.allowedSources.includes(payload.context?.source)
		) {
			return null;
		}
...
export function validateIntegrationConfig(config) {
	...
	if (config.eventProcessing) {
		...
		if (
			ep.maxConcurrentHandlers &&
			(typeof ep.maxConcurrentHandlers !== 'number' ||
				ep.maxConcurrentHandlers <= 0)
		) {
		...
		if (
			ep.handlerTimeout &&
			(typeof ep.handlerTimeout !== 'number' || ep.handlerTimeout <= 0)
		) {
		...
		if (
			ep.batchSize &&
			(typeof ep.batchSize !== 'number' || ep.batchSize <= 0)
		) {
	...
	if (config.retry) {
		...
		if (
			...
			!['exponential', 'linear', 'fixed'].includes(retry.backoffStrategy)
		) {
			errors.push(
...
// Export enhanced event system components
export { EventEmitter } from './event-emitter.js';
export { EventBus } from './event-bus.js';
export { EventQueue, PRIORITY, QUEUE_STATE } from './event-queue.js';
export { EnhancedIntegrationManager } from './enhanced-integration-manager.js';

// Export for testing
export const __testing = {
	getGlobalEventManager: () => globalEventManager,
	setGlobalEventManager: (manager) => {
