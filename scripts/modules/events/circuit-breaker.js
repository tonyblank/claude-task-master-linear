 ...
 * in the integration system. It monitors failure rates and automatically opens
 * the circuit when failures exceed thresholds, preventing further calls to failing services.
 */
...
export class CircuitBreaker {
	...
	forceState(state, reason = 'manual') {
		...
		log('warn', `Circuit breaker force state change to ${state}: ${reason}`);
...
export class CircuitBreakerRegistry {
	...
	getBreaker(name, config = {}) {
		if (!this.breakers.has(name)) {
			...
			log('debug', `Created circuit breaker: ${name}`);
		}
