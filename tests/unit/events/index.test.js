...
describe('Event System Index', () => {
  ...
  describe('validateIntegrationConfig', () => {
    ...
    test('should validate eventProcessing config', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('maxConcurrentHandlers must be a positive number');
      expect(result.errors).toContain('handlerTimeout must be a positive number');
    ...
    test('should validate retry config', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('retry.maxAttempts must be a positive number');
      expect(result.errors).toContain('retry.baseDelay must be a positive number');
      expect(result.errors).toContain('retry.backoffStrategy must be one of: exponential, linear, fixed');
    });
