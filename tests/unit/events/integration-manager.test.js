...
import { EVENT_TYPES, createEventPayload } from '../../../scripts/modules/events/types.js';
...
describe('IntegrationManager', () => {
  ...
  describe('initialization', () => {
    ...
    test('should not initialize twice', async () => {
      ...
      const warnFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[WARN]') && arg.includes('IntegrationManager is already initialized'))
      );
  ...
  describe('integration registration', () => {
    ...
    test('should reject non-BaseIntegrationHandler instances', () => {
      ...
      expect(() => manager.register(invalidIntegration))
  ...
  describe('integration unregistration', () => {
    ...
    test('should handle unregistering non-existent integration', async () => {
      ...
      // Check that warning was logged
      const warnFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[WARN]') && arg.includes('Integration non-existent is not registered'))
      );
      expect(warnFound).toBe(true);
    });
  ...
  describe('event emission', () => {
    ...
    test('should emit and handle events successfully', async () => {
      ...
      expect(testIntegration.handledEvents[0].type).toBe(EVENT_TYPES.TASK_CREATED);
    ...
    test('should not emit events when not initialized', async () => {
      ...
      await uninitializedManager.emit(EVENT_TYPES.TASK_CREATED, {}, mockContext);
      ...
      // Check that warning was logged
      const warnFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[WARN]') && arg.includes('IntegrationManager not initialized'))
      );
      expect(warnFound).toBe(true);
    });
    ...
    test('should reject events when shutting down', async () => {
      ...
      // Check that warning was logged
      const warnFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[WARN]') && arg.includes('IntegrationManager is shutting down'))
      );
      expect(warnFound).toBe(true);
    });
    ...
    test('should update statistics', async () => {
      ...
      await manager.emit(EVENT_TYPES.TASK_CREATED, {
        taskId: 'task-123',
        task: { id: 'task-123' },
        tag: 'master'
      }, mockContext);
      ...
      expect(updatedStats.eventsProcessed).toBe(initialStats.eventsProcessed + 1);
    });
  });
  ...
  describe('event handler execution', () => {
    ...
    test('should isolate handler failures', async () => {
      ...
      const errorFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[ERROR]') && arg.includes('Handler failed'))
      );
    ...
    test('should handle handler timeouts', async () => {
      ...
      const errorFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && (arg.includes('[ERROR]') && arg.includes('Handler timeout')) || arg.includes('timeout'))
      );
  ...
  describe('middleware', () => {
    test('should execute middleware before handlers', async () => {
      ...
      expect(testIntegration.handledEvents[0].payload.middleware).toBe('processed');
    });
    ...
    test('should continue processing if middleware throws', async () => {
      ...
      expect(testIntegration.handledEvents.length).toBeGreaterThan(0);
      expect(testIntegration.handledEvents[0].payload.secondMiddleware).toBe(true);
    });
  });
  ...
  describe('wildcard handlers', () => {
    test('should match wildcard handlers', async () => {
      ...
      expect(wildcardHandler.calls[0][1]).toEqual(expect.objectContaining({
        taskId: 'task-123'
      }));
    });
    ...
    test('should match pattern handlers', async () => {
      ...
      await manager.emit(EVENT_TYPES.TASK_STATUS_CHANGED, {
        taskId: 'task-123',
        task: { id: 'task-123' },
        oldStatus: 'pending',
        newStatus: 'done',
  ...
  describe('shutdown', () => {
    ...
    test('should not shutdown twice', async () => {
      ...
      const shutdownCompletedFound = logCalls.some(call => 
        call.some(arg => typeof arg === 'string' && arg.includes('[INFO]') && arg.includes('IntegrationManager shutdown completed'))
      );
  ...
  describe('event validation', () => {
    ...
    test('should accept valid event payloads', async () => {
      ...
      await expect(
        manager.emit(EVENT_TYPES.TASK_CREATED, {
          taskId: 'task-123',
          task: { id: 'task-123' },
          tag: 'master'
        }, mockContext)
