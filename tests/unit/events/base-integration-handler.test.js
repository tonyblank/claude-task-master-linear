...
describe('BaseIntegrationHandler', () => {
	...
	describe('constructor', () => {
		...
		test('should prevent direct instantiation of abstract class', () => {
			expect(() => new BaseIntegrationHandler('test', '1.0.0')).toThrow(
			...
			const warnFound = logCalls.some((call) =>
				call.some(
					(arg) =>
						typeof arg === 'string' &&
						arg.includes('[WARN]') &&
						arg.includes('is already initialized')
				)
			);
	...
	describe('shutdown', () => {
		...
		test('should timeout waiting for active operations', async () => {
			const quickTimeoutHandler = new TestHandler('quick-timeout', {
				timeout: 50
			});
			...
			const warnFound = logCalls.some((call) =>
				call.some(
					(arg) =>
						typeof arg === 'string' &&
						arg.includes('[WARN]') &&
						arg.includes('shutdown with 1 active operations')
				)
			);
			expect(warnFound).toBe(true);
		});
	...
	describe('event handling', () => {
		...
		test('should not handle events when disabled', async () => {
			...
			const result = await disabledHandler.handleEvent(
				EVENT_TYPES.TASK_CREATED,
				{}
			);
		...
		test('should convert event types to method names correctly', async () => {
			...
			const testCases = [
				...
				{
					eventType: 'subtask:status:changed',
					expectedMethod: 'handleSubtaskStatusChanged'
				},
				{
					eventType: 'dependency:added',
					expectedMethod: 'handleDependencyAdded'
				}
			];
	...
	describe('error checking', () => {
		test('should identify retryable errors correctly', () => {
			...
			expect(handler._isRetryableError(connResetError, retryableConfig)).toBe(
				true
			);

			// Test message matching
			const timeoutError = new Error('Request TIMEOUT occurred');
			expect(handler._isRetryableError(timeoutError, retryableConfig)).toBe(
				true
			);
			...
			expect(handler._isRetryableError(rateLimitError, retryableConfig)).toBe(
				true
			);
